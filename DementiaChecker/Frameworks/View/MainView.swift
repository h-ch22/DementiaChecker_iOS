//
//  MainView.swift
//  DementiaChecker
//
//  Created by Changjin Ha on 1/28/24.
//

import SwiftUI

struct MainView: View {
    @State var selectedIndex = 0
    @State private var showModal = false
    
    @EnvironmentObject var userManagement: UserManagement
    
    private let icons = ["house.fill", "map.fill", "plus", "calendar.badge.clock", "ellipsis.circle.fill"]
    private let titles = ["Home", "Hospital", "Inspection", "History", "More"]
    
    var body: some View {
        if UIDevice.current.userInterfaceIdiom == .pad {
            if #available(iOS 18.0, *) {
                TabView {
                    ForEach(0..<5, id: \.self){ idx in
                        Tab(titles[idx], systemImage: icons[idx]){
                            switch idx{
                            case 0: HomeView()
                            case 1: HospitalMapView()
                            case 2: InspectionView(showToolbar: false)
                                    .environmentObject(userManagement)
                            case 3: HistoryView()
                            case 4: MoreView()
                            default: HomeView()
                            }
                        }
                    }
                }.tabViewStyle(.sidebarAdaptable)
            } else {
                NavigationSplitView(sidebar: {
                    VStack{
                        ForEach(0..<5, id: \.self){ idx in
                            Button(action: {
                                if idx != 2{
                                    selectedIndex = idx
                                }
                            }){
                                if idx != 2{
                                    HStack{
                                        Image(systemName: icons[idx])
                                            .foregroundStyle(selectedIndex == idx ? Color.white : Color.txt)
                                        
                                        Spacer().frame(width: 5)
                                        
                                        Text(titles[idx])
                                            .foregroundStyle(selectedIndex == idx ? Color.white : Color.txt)
                                        
                                        Spacer()
                                    }.padding(10)
                                        .background(
                                            RoundedRectangle(cornerRadius: 15).foregroundStyle(
                                                selectedIndex == idx ? Color.accentColor : Color.clear
                                            )
                                        )
                                }
                            }
                        }
                        
                        Spacer()
                    }.navigationTitle(Text("Dementia Checker"))
                        .toolbar{
                            Button(action: {
                                self.showModal = true
                            }){
                                Image(systemName: "plus")
                            }
                        }
                        .sheet(isPresented: $showModal, content: {
                            InspectionView(showToolbar: false)                                .environmentObject(userManagement)
                        })
                        .padding(10)
                }, detail: {
                    switch selectedIndex {
                    case 0:
                        HomeView()
                        
                    case 1:
                        HospitalMapView()
                        
                    case 3:
                        HistoryView()
                        
                    case 4:
                        MoreView()
                        
                    default:
                        HomeView()
                    }
                })
            }
        } else{
            NavigationStack{
                VStack{
                    ZStack{
                        switch selectedIndex{
                        case 0:
                            HomeView()
                            
                        case 1:
                            HospitalMapView()
                            
                        case 3:
                            HistoryView()
                            
                        case 4:
                            MoreView()
                            
                        default:
                            HomeView()
                        }
                    }
                    
                    Spacer()
                    
                    HStack{
                        ForEach(0..<5, id:\.self){number in
                            Spacer()
                            
                            Button(action: {
                                if number == 2{
                                    self.showModal = true
                                }
                                
                                else{
                                    selectedIndex = number
                                }
                            }){
                                if number == 2{
                                    Image(systemName: icons[number])
                                        .font(.system(
                                            size: 25,
                                            weight: .regular,
                                            design: .default
                                        ))
                                        .foregroundStyle(Color.txt)
                                        .frame(width : 60, height : 60)
                                        .background(
                                            Circle()
                                                .fill(Color.background)
                                                .shadow(color: Color.shadowStart.opacity(0.2), radius: 10, x: 10, y: 10)
                                                .shadow(color: Color.shadowEnd.opacity(0.7), radius: 10, x: -5, y: -5)
                                        )
                                }
                                
                                else{
                                    VStack{
                                        Image(systemName: icons[number])
                                            .font(.system(
                                                size: 20,
                                                weight: .regular,
                                                design: .default
                                            ))
                                            .foregroundStyle(selectedIndex == number ? Color.txt : Color.gray)
                                        
                                        Spacer().frame(height: 5)
                                        
                                        if selectedIndex == number{
                                            Text(titles[number])
                                                .font(.caption)
                                                .foregroundStyle(Color.txt)
                                        }
                                    }.padding(selectedIndex == number ? 10 : 0)
                                        .background(RoundedRectangle(cornerRadius: selectedIndex == number ? 15 : 0).foregroundStyle(selectedIndex == number ? Color.background : Color.clear).opacity(0.6))
                                }
                                
                            }
                            
                            Spacer()
                        }
                    }.padding(10)
                        .background(.ultraThinMaterial)
                        .clipShape(RoundedRectangle(cornerRadius: 15))
                        .padding([.horizontal], 5)
                        .animation(Animation.easeInOut(duration: 0.5), value: true)
                }
                
                .sheet(isPresented: $showModal, content: {
                    InspectionView(showToolbar: false)
                        .environmentObject(userManagement)
                })
                .background(Color.background.ignoresSafeArea(.all, edges: [.top, .bottom]))
                
            }
            .toolbar(.hidden)
        }
        
    }
}

#Preview {
    MainView()
        .environmentObject(UserManagement())
}
